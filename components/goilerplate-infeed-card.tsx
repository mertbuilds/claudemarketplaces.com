...
export function GoilerplateInFeedCard() {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && typeof window.op === "function") {
          window.op!("track", "infeed_card_viewed", { card: "goilerplate" });
          observer.disconnect();
        }
      },
      { threshold: 0.5 }
      ...
      <CardHeader>
        <div className="flex flex-col gap-2">
          <div className="flex items-start justify-between gap-2">
            <CardTitle className="text-xl line-clamp-2 flex-1 min-w-0 leading-7">
              <a
                ...
                onClick={() => { if (typeof window.op === "function") window.op!("track", "infeed_card_clicked", { card: "goilerplate" }); }}
              >
                Goilerplate
              </a>
            </CardTitle>
          ...
          <div className="flex items-center gap-1 text-muted-foreground">
            ...
            <span className="text-sm">Go SaaS boilerplate</span>
          </div>
        </div>
        <CardDescription className="line-clamp-3">
          Production-ready Go boilerplate for SaaS. Auth, billing, emails, and more — save 300+ hours of setup. Pay once, build unlimited.
        </CardDescription>
      </CardHeader>
      <CardContent>
